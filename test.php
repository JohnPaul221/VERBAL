    ...
    <script>
        ...
        const wordBank = {
            ...
            intermediate: [
                ...
                { sentence: "Gravity holds us on the ground now", phonemes: ["g", "r", "a", "v", "i", "t", "ee"], example: "The force of gravity holds us down on the ground." },
        ...
        function getUsEnglishVoice() {
            ...
            return voices.find(v => v.name === 'Google US English' || v.lang === 'en-US');
        }
        ...
        function loadNextWord() {
            ...
            feedbackMessage.textContent = "Magsalita para makita ang sentence!";
            ...
            const imgKey = currentWord.word.toLowerCase();
            if (imageLibrary[imgKey]) {
                document.getElementById("wordImage").src = imageLibrary[imgKey];
                imageBox.style.backgroundImage = `url('${imageLibrary[imgKey]}')`;
            }
        ...
        function initRecognition() {
            if (!('webkitSpeechRecognition' in window)) {
                alert("Ang browser na ito ay hindi suportado ang Voice Recognition. Gumamit ng Google Chrome.");
                return;
            }
            ...
            recognition.onresult = (event) => {
                ...
                for (let i = event.resultIndex; i < event.results.length; ++i) {
                    ...
                    if (event.results[i].isFinal) {
                        ...
                        // Real-time check: Kung tama na agad kahit hindi pa 'final'
                        if (interimTranscript.toLowerCase().trim() === currentWord.word.toLowerCase()) {
        ...
        function processVoiceInput(spokenText) {
            transcriptEl.textContent = spokenText;
            const score = ratePronunciation(spokenText, currentWord.word);
            if (score === 0) return;
        ...
        function ratePronunciation(spoken, target) {
            ...
            if (target.includes(spoken) && spoken.length > 1) return 3;
            return 1;
        }
        ...
        function stopMicLogic() {
            ...
            statusEl.textContent = "Tap Mic to Read";
        ...
        function init() {
            ...
            nextBtn.addEventListener("click", () => {
                allProgress[currentDifficulty].word_index++;
                saveProgressToDB();
                loadNextWord();
            });
